...
//
//    @Query(value = "select a.title, a.created_at, a.views, a.likes, b.nick_name from board a join member b ON a.member_id = b.member_id where board_id IN (select board_id  from board_category where category_id IN (SELECT category_id from category where musical_id = :musicalId))", nativeQuery = true)
...
////    List<MusicalBoards> findBoardByMusicalId(@Param("musicalId") Long musicalId);
//
//    @Query(value = "SELECT category_name from category where musical_id = :musicalId", nativeQuery = true)
